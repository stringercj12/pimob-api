...
public class UserUpdateStatusUseCase implements IUserUpdateStatusUseCase {
    ...
    public void execute(UserUpdateStatusRequest request) {
        ...
        if (!User.StatusUsuario.exists(request.statusUsuario().toString())) {
            throw new ValidationException("Informe um status válido");
        }

        if (request.statusUsuario() != User.StatusUsuario.REJEITADO || request.statusUsuario() != User.StatusUsuario.PENDENTE) {
            user.setAtivo(true);
        }
